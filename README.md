...
- タスクに優先度を設定できる
  ...
  - `low`
- タスクにカテゴリを設定できる
- タスクに任意の詳細メモを登録・編集できる
- タスクに期限日を設定できる
- メモがあるタスクはカード上で表示し、長いメモは省略表示から展開できる
...
- タスク一覧をJSONファイルとしてエクスポートできる
- JSONファイルからタスク一覧を置き換えまたは追加でインポートできる
- ライトモード / ダークモードを切り替えできる
- 初回表示時はOSのカラーモードを参考にし、ユーザーが選択したテーマを優先できる

### 検索・フィルタ・並び替え

- キーワードでタスク名・カテゴリ・メモを検索できる
- 状態でフィルタできる
  - `all`
  - `todo`
  - `inProgress`
...
- タスク一覧を`task-board-tasks.json`としてバックアップできる
- JSONインポートではメタ情報付き形式と単純な`Task[]`形式の両方を読み込める
- ページを再読み込みしてもタスクが残る
...
- 既存の保存データに`description`がない場合は空文字を補完して読み込む
- 不正な`description`は空文字に丸める
- 不正JSONや壊れたタスクデータを読み込んだ場合は、現在のタスクを変更せずエラーを表示する
...
type TaskImportMode = 'replace' | 'append'
...
- 古い保存データに`description`がない場合は空文字を補完する
- `description`が文字列でない場合は空文字として扱う
- 基本項目が壊れている場合は警告を表示し、初期状態で表示する

### JSONエクスポート / インポート

- エクスポートファイル名: `task-board-tasks.json`
- 標準エクスポート形式は`version`、`exportedAt`、`app`、`tasks`を持つJSONにする
- 互換性のため、インポート時は単純な`Task[]`形式も受け付ける
- インポート方法は「現在のタスクを置き換え」と「現在のタスクに追加」から選べる
- インポート対象が0件の場合は、現在のタスクを変更せず注意メッセージを表示する
- 不正なタスクが1件でも含まれる場合は、部分的に取り込まずインポート全体を失敗にする
- 追加インポートではID重複があるタスクに新しいIDを振り、既存タスクを上書きしない
- 置き換えインポートではIDを維持し、ファイル内でID重複がある場合は失敗にする
...
- `TaskImportExport`: JSONエクスポート / インポート
- `TaskList`: 表示対象タスク一覧
...
- `createTaskId`: タスクIDを生成する
- `createTask`: 入力値から`Task`を作成する
- `applyTaskUpdate`: 編集内容を既存タスクに反映する
- `normalizeDescription`: メモの保存互換性を保つため文字列以外を空文字へ丸める
- `normalizeTask`: localStorage読み込みとJSONインポートで共通利用するタスク正規化
- `normalizeTasks`: タスク配列をall-or-nothingで正規化する
- `createTaskExportFile`: メタ情報付きのエクスポートJSONを作成する
...
2. 必要に応じて状態、カテゴリ、優先度、期限日、メモを設定する
3. 「追加」ボタンでタスクを登録する
4. タスクカードのクイック操作ボタンで未着手 / 進行中 / 完了を切り替える
5. 「編集」ボタンでタスク名、メモ、状態、カテゴリ、優先度、期限日を更新する
6. 「削除」ボタンで不要なタスクを削除する
7. 検索欄でタスク名・カテゴリ・メモを検索し、状態、優先度フィルタで表示するタスクを絞り込む
8. 並び替えselectで作成日、期限、優先度、未完了優先の順に並べ替える
9. リスト / ボードの表示形式を切り替える
10. ヘッダーのライト / ダーク切り替えでテーマを変更する
11. 「データ管理」からタスク一覧をJSONファイルとして書き出す
12. JSONファイルを選び、現在のタスクを置き換えるか追加するかを選んでインポートする
13. 追加・更新・削除・インポートした内容とテーマ設定は`localStorage`に保存され、ページ再読み込み後も残る
...
- タスク詳細メモを任意入力として追加し、古い保存データでは空文字に補完している
- データのバックアップ・復元機能としてJSONエクスポート / インポートを実装している
- 不正JSONや壊れたタスクデータを読み込んでも、既存データを変更しない防御的な設計にしている
- 検索対象をタスク名・カテゴリ・メモへ拡張している
...
5. 検索・フィルタ欄の下にデータ管理エリアを置く
6. リスト表示またはボード表示でタスク、メモ、状態、期限日、期限通知風バッジを表示する
7. タスクがない場合やフィルタ結果がない場合は、用途に合わせた空状態を表示する
...
- メモが空でもタスクを追加・編集できること
- 長いメモをカード上で省略表示し、必要に応じて展開できること
- 空タイトルでは編集保存できないこと
...
- 古い保存データでも`description: ''`として復元されること
- 文字列でない`description`が空文字として復元されること
...
- タスク一覧を`task-board-tasks.json`としてエクスポートできること
- エクスポートしたJSONを再インポートできること
- 置き換えインポート後に検索・フィルタ・並び替えがリセットされること
- 追加インポート後に検索・フィルタ・並び替えが維持されること
- 追加インポート時にID重複で既存タスクが上書きされないこと
- 空のJSONタスク配列をインポートしても現在のタスクが消えないこと
- 不正JSONや配列でないJSONでエラーが表示され、現在のタスクが壊れないこと
- タスク名・カテゴリ・メモの検索とフィルタを同時に使えること
...
14. タスク詳細メモとメモを含む検索を追加する
15. JSONエクスポート / インポートによるバックアップ・復元を追加する
